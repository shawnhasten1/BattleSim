...
export function ActionsTab({ definition, compendium }: { combatant: CombatantState; definition: CreatureDefinition; compendium?: Compendium }) {
  ...
  const [libKind, setLibKind] = useState<"all" | "weapon" | "spell">("all");
  const [libQuery, setLibQuery] = useState("");
  const [maName, setMaName] = useState("Multiattack");
  const [maPicks, setMaPicks] = useState<string[]>([]);
  const [maCount, setMaCount] = useState(2);
  ...
  const libraryResults = useMemo(() => {
    ...
    return libQuery.trim()
      ? searchSrd(libQuery, kind)
      : searchSrd("", kind).slice(0, 24);
  }, [libQuery, libKind]);
  ...
  function editWeapon(weapon: WeaponDefinition) {
    openEdit({ kind: "weapon", id: weapon.id }, weaponDraftFromDefinition(weapon));
  }
  function editSpell(spell: SpellDefinition) {
    openEdit({ kind: "spell", id: spell.id }, spellDraftFromDefinition(spell));
  }
  ...
  function attachFromLibrary(kind: "weapon" | "spell", id: string) {
    if (kind === "weapon") attachSrdWeapon(definition.id, id);
    else attachSrdSpell(definition.id, id);
    setAddOpen(false);
  }
            ...
            {addTab === "library" ? (
              <>
                ...
                <p style={{ margin: 0, fontSize: 10.5, color: "var(--ui-text-dim)" }}>
                  Click or drag onto the sheet. {SRD_WEAPONS.length} weapons, {SRD_SPELLS.length} spells.
                </p>
              </>
            ) : null}
        ...
        <div className={styles.group}>
          ...
          <div className={styles.riderCard} style={{ marginTop: 6 }}>
            ...
            <div className={styles.chips}>
              {attackChoices.map((action) => (
                <button
                  key={action.id}
                  type="button"
                  className={maPicks.includes(action.id) ? styles.chipOn : undefined}
                  onClick={() => setMaPicks((p) => p.includes(action.id) ? p.filter((x) => x !== action.id) : [...p, action.id])}
                >
                  {action.name}
                </button>
              ))}
            ...
            <div className={styles.riderRow}>
              <label className={styles.fieldInlineLabel}>Times each
                <input type="number" min={1} value={maCount} onChange={(e) => setMaCount(Math.max(1, Number(e.target.value) || 1))} />
              </label>
            </div>
            <button
              type="button" className={styles.riderAdd}
              onClick={() => { if (maPicks.length) { addMultiattack(definition.id, { name: maName, actionIds: maPicks, count: maCount }); setMaPicks([]); } }}
            >
              + Add multiattack
            </button>
          </div>
        ...
        return (
          <div key={title as string} className={styles.group}>
            ...
            {actions.map((action) => row(
              action.id, action.name, describeAction(action, definition), action.automationSupport,
